...
- `.env.local`'s actual Supabase var names don't match `architecture.md`'s documented `SUPABASE_URL` / `.env.example`: it currently has `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (both unused by this app's server-only design — see Session Notes) alongside the correctly-named `SUPABASE_SERVICE_ROLE_KEY` and a migration-only `SUPABASE_DB_URL` (not used by the running app, only by one-off migration scripts). Needs reconciling — likely renaming/adding a plain `SUPABASE_URL` and updating `.env.example` — before Spec 05 writes `lib/supabase/client.ts` and needs to read the project URL server-side.
