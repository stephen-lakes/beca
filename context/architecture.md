...
| KB source | WHO fact sheets, 30 topics | — | Fetched once, ingested at build time — see `data/kb_topics.json`. Expanded 12 → 21 post-launch (architecture audit, 2026-08-27), then 21 → 28 (capability router pass, 2026-08-28), then 28 → 29 (immunization-schedule addition, 2026-08-29), then 29 → 30 (seasonal-influenza addition, 2026-09-03) to close real KB-coverage gaps; see `progress-tracker.md`. |
...
│   │                             # ServiceResultsCard (Spec 20), useAutoScroll +
│   │                             # JumpToLatestButton (auto-scroll UX fix, 2026-08-29),
│   │                             # ConversationalReplyBubble (Spec 26)
...
- **Capability router as of Spec 20**: `preparation_checklists` (structured, deterministic — never vector search) and `directory_entries.services` (service-based, additive to the existing escalation `category` column) support `healthcare_preparation` and `service_navigation` respectively. `health_education`/`preventive_health`/`disease_information`/`when_to_seek_care`/`medication_safety` route through the unmodified hybrid RAG pipeline above — see `context/specs/20-capability-router-and-navigation.md`. **`out_of_scope` no longer does, as of Spec 25 below.**
...
- **Conversational intents as of Specs 25–26**: six capability values (`greeting`/`farewell`/`thanks`/`help_request`/`general_conversation`/`out_of_scope`) dispatch directly to a fifth response shape (`ConversationalResponseSchema`, fixed template copy, `lib/ai/schema.ts`) — never `searchKb`, never `generateAnswer`, never a directory lookup. `out_of_scope` previously fell through to the RAG pipeline above and returned the same message as a genuine KB-coverage gap; it no longer does. `ChatThread.tsx` renders this shape via `ConversationalReplyBubble` (Spec 26) — see `context/specs/25-conversational-intents-and-out-of-scope-redirect.md` and `context/specs/26-conversational-intents-ui.md`.
