...
- **Conversational reply bubble** (Spec 26) — a greeting, farewell, thank-you, help-request, or off-topic redirect (Spec 25). Bubble-width, styled like a plain, successful answer: solid `line` border, `card` background — deliberately not the no-grounded-information bubble's dashed/italic treatment (this isn't a refusal), and deliberately calmer than the clarification/service-results cards (no icon, no `role="status"`, no heading label) — this is ordinary conversation, not a distinct app state announcing itself.
