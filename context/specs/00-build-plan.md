...
| 19 | Hybrid retrieval, query expansion, fallback diagnostics | Postgres full-text search alongside the existing pgvector search (new migration), keyword-metadata-driven query expansion, and A–E fallback-outcome logging in `app/api/chat/route.ts` — no UI, no response-shape change. Implemented and verified 2026-08-27/28, see `context/specs/19-hybrid-retrieval-fallback-diagnostics.md`. | 05 |
| 20 | Capability router: preventive health, healthcare preparation, service navigation | New capability classifier (`lib/ai/classify-capability.ts`) after the existing safety layer; new `preparation_checklists` table + deterministic lookup for `healthcare_preparation`; new `directory_entries.services` column + deterministic lookup for `service_navigation`; new `ServiceResultsCard` UI; 7 new KB topics; 5 evaluation datasets + a capability-eval runner. Implemented and verified live 2026-08-28, see `context/specs/20-capability-router-and-navigation.md`. | 06, 17, 19 |
| 23 | Conversation context resolution — resolver + API route | A non-self-contained follow-up (e.g. "what are the causes") is resolved against the thread's own recent history into a standalone query before the urgency classifier, capability classifier, and retrieval all run (the deterministic red-flag check is the one exception — always raw message). New `lib/ai/resolve-context.ts`; `app/api/chat/route.ts` request contract gains an optional, client-supplied `recentHistory` field (server stays stateless). Verified via curl, no UI. Implemented and verified live 2026-08-28, see `context/specs/23-conversation-context-resolution.md` — Decision 1 was revised mid-implementation after live testing showed the initial, more conservative placement (resolution after the safety layer) only fixed the reported bug 2/3 of the time. | 06, 17, 20 |
| 24 | Conversation context resolution — UI wiring | `ChatThread.tsx` sends the thread's own bounded recent-turn history (flattened to plain text per turn) on every request, so Spec 23's resolver has something to resolve against. Same relationship 18 had to 17. Implemented 2026-08-29, see `context/specs/24-conversation-context-resolution-ui.md`. | 23 |
...
## Total units: 18 implemented (14 original + unit 19, hybrid retrieval + unit 20, capability router + unit 23, conversation context resolution + unit 24, conversation context resolution UI wiring)
